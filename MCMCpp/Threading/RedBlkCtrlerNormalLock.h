...
********************************************************************************
*******************************************************************************/
#ifndef MCMCPP_THREADING_REDBLKCTRLERNORMALLOCK_H
#define MCMCPP_THREADING_REDBLKCTRLERNORMALLOCK_H
...
class RedBlackCtrler<ParamType, EndOfStepAction, false>
{
public:
    /*!
     ...
     */
    RedBlackCtrler(int numThreads, Chain::Chain<ParamType>& chainRef, EndOfStepAction* ptr);
    ...
    /*!
     * \brief workerMidStepWait Used by workers to wait for the worker status to change from ProcessRed, specialized to handle using or not using spinlocks
     */
    void workerMidStepWait();
    
    /*!
     * \brief workerMidStepWait Used by workers to wait for the worker status to change from ProcessBlack, specialized to handle using or not using spinlocks
     */
...
private:
    ...
    std::mutex midStepMutex; ///<Mutex for when midStep waiting when we are not using spin-locks
    std::condition_variable midStepWait; ///<Condition variable  for when midStep waiting when we are not using spin-locks
    
    //worker end step control
    std::atomic_int numWorkersAtEndStep; ///<Number of workers that are waiting at the mid step point
    std::mutex endStepMutex; ///<Mutex for when endStep waiting when we are not using spin-locks
    std::condition_variable endStepWait; ///<Condition variable  for when endStep waiting when we are not using spin-locks
};

template<class ParamType, class EndOfStepAction>
RedBlackCtrler<ParamType, EndOfStepAction, false>::
RedBlackCtrler(int numThreads, Chain::Chain<ParamType>& chainRef, EndOfStepAction* ptr):
    ...
    numWorkersAtMidStep(0), midStepMutex(), midStepWait(),
    numWorkersAtEndStep(0), endStepMutex(), endStepWait()
{
    //std::cout<<std::this_thread::get_id()<<" just finished constructor"<<std::endl;
}

template<class ParamType, class EndOfStepAction>
void RedBlackCtrler<ParamType, EndOfStepAction, false>::terminateWorkers()
{
    //std::cout<<std::this_thread::get_id()<<" starting terminate"<<std::endl;
    {
    ...
    //std::cout<<std::this_thread::get_id()<<" ending terminate"<<std::endl;
}

template<class ParamType, class EndOfStepAction>
void RedBlackCtrler<ParamType, EndOfStepAction, false>::runSampling(int numSteps, int skipPoints)
{
    //std::cout<<std::this_thread::get_id()<<" starting runSampling"<<std::endl;
    ...
    //std::cout<<std::this_thread::get_id()<<" ending runSampling"<<std::endl;
}

template<class ParamType, class EndOfStepAction>
void RedBlackCtrler<ParamType, EndOfStepAction, false>::workerWaitForTask()
{
    //std::cout<<std::this_thread::get_id()<<" starting waitForTask"<<std::endl;
    ...
    //std::cout<<std::this_thread::get_id()<<" ending waitForTask"<<std::endl;
}

template<class ParamType, class EndOfStepAction>
void RedBlackCtrler<ParamType, EndOfStepAction, false>::workerMidStepWait()
{
    //std::cout<<std::this_thread::get_id()<<" starting midStepWait"<<std::endl;
    std::unique_lock<std::mutex> lock(midStepMutex);
    ...
    //std::cout<<std::this_thread::get_id()<<" ending midStepWait"<<std::endl;
}

template<class ParamType, class EndOfStepAction>
void RedBlackCtrler<ParamType, EndOfStepAction, false>::workerEndStepWait()
{
    //std::cout<<std::this_thread::get_id()<<" starting endStepWait"<<std::endl;
    ...
    std::unique_lock<std::mutex> lock(endStepMutex);
    if(temp == lastThread)
    ...
    //std::cout<<std::this_thread::get_id()<<" ending endStepWait"<<std::endl;
}

template<class ParamType, class EndOfStepAction>
void RedBlackCtrler<ParamType, EndOfStepAction, false>::workerAckTerminate()
{
    //std::cout<<std::this_thread::get_id()<<" starting workerAckTerminate"<<std::endl;
    ...
    //std::cout<<std::this_thread::get_id()<<" ending workerAckTerminate"<<std::endl;
}

}
}
#endif  //MCMCPP_THREADING_REDBLKCTRLERNORMALLOCK_H
